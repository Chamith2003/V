let completedRatings = parseInt(document.getElementById('completedCount').textContent)||0;
        const totalRatings =parseInt(document.getElementById('totalCount').textContent)||0;
        ...
        document.querySelectorAll('.starRating').forEach(starRating => {
            const stars = starRating.querySelectorAll('.star');
            const card = starRating.closest('.taskRatingCard');
...
document.querySelectorAll('.submitRating').forEach(btn => {//btn is the current element in the loop
    btn.addEventListener('click', function() {
        const card = this.closest('.taskRatingCard');//closest DOM ancestor=finds the card containing this button
        const starRating = card.querySelector('.starRating');//get div with all stars
        const rating = parseInt(starRating.dataset.rating);
        const comment = card.querySelector('.commentInput').value;//attribute starting with data-xxx is called a data attribute , In JavaScript you access it with element.dataset.xxx 
        const taskId = card.dataset.task;//get the task details to store from data-task
        ...
        if (rating > 0) {
            ...
            formData.append('task_id', taskId);//“Append” here literally means stick it to the end of the FormData collection, ready to be sent in the POST request
            //If the key already exists, it adds another value for the same key (FormData can hold multiple values per key)
            formData.append('rating', rating);//formData.append(key, value) adds a new key-value pair to the FormData object
            formData.append('comment', comment);//append key-value pairs (like task ID, rating, comment) this way we can send data without a form tag thruh AJAX

            fetch('/V/router.php?module=rating&action=submittaskrating', {//send an HTTP request to the server
            ...
            .then(data => {
                if (data.success) {// Mark as completed
                    ...
                    card.classList.add('completed');//visually mark the card (tick + color change)
